...
Before recording an account request for a group with [approve](approve.md), check to see if the group already has an account. Be
sure to use different forms and abbreviations of the group's name to maximize
the chance that a group will be found.

This script will search [LDAP](../infrastructure/ldap.md) for case-insensitive
